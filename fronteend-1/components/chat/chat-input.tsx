...
export function ChatInput({ onSend, disabled }: ChatInputProps) {
  ...
  const handleSend = useCallback(() => {
    ...
    onSend(trimmed, fileData.length > 0 ? fileData : undefined);
    setInput("");
    setFiles([]);
        ...
        {files.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-3 p-3 rounded-xl glass-card">
            {files.map((f, i) => (
              <div
                key={i}
                className="relative group flex items-center gap-2 px-3 py-2 rounded-lg bg-white/[0.04] border border-border/30"
              >
                {f.url ? (
                  ...
                  <div className="w-8 h-8 rounded bg-ayur-gold/10 flex items-center justify-center">
                    {f.file.type.includes("pdf") ? (
                      ...
                      <ImageIcon className="w-4 h-4 text-ayur-gold" />
                    )}
                  </div>
                )}
        ...
        <div className="relative glass-card-strong rounded-2xl transition-all duration-300 focus-within:shadow-[0_0_0_1px_rgba(201,169,110,0.2)]">
          ...
          <div className="flex items-end gap-2 p-3">
            ...
            <button
              ...
              disabled={disabled}
              className="flex-shrink-0 w-9 h-9 rounded-xl flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-white/5 transition-all duration-200 mb-0.5"
            ...
            <textarea
              ...
              disabled={disabled}
              placeholder="Ask about Ayurvedic herbs, doshas, remedies..."
              rows={1}
              className="flex-1 bg-transparent border-none outline-none resize-none text-sm text-foreground placeholder:text-muted-foreground/50 leading-relaxed max-h-[200px] py-1.5"
            />
            ...
            <button
              ...
              className={`flex-shrink-0 w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-300 mb-0.5 ${
                hasContent
                  ? "bg-ayur-gold text-background hover:bg-ayur-amber"
